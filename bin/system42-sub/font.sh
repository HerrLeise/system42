...
set_font() {
	...
	if ! $(fc-list | grep -i "$font_name" >/dev/null); then
		...
		cd -
		clear
		source $SYSTEM42_PATH/ascii.sh
	fi

	gsettings set org.gnome.desktop.interface monospace-font-name "$font_name 10"
	cp "$SYSTEM42_PATH/configs/alacritty/fonts/$file_name.toml" ~/.config/alacritty/font.toml
...
"Meslo")
	...
	;;
"> Change size")
	source $SYSTEM42_PATH/bin/system42-sub/font-size.sh
	exit
	;;
esac
