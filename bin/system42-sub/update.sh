...
CHOICES=(
	"system42      Update system42 itself and run any migrations"
...
if [[ "$CHOICE" == "<< Back"* ]] || [[ -z "$CHOICE" ]]; then
	...
	case "$INSTALLER" in
	"system42") INSTALLER_FILE="$SYSTEM42_PATH/bin/system42-sub/migrate.sh" ;;
	"ollama") INSTALLER_FILE="$SYSTEM42_PATH/install/terminal/optional/app-ollama.sh" ;;
	*) INSTALLER_FILE="$SYSTEM42_PATH/install/terminal/app-$INSTALLER.sh" ;;
	esac
